...
Known Issue [2]: When in some shop menus, only the menu options for items in the shop are translated. Menu actions (Value, Sell) for items in the user's inventory are not translated, but item names still are. Extremely low priority to fix because [1] it's a very minor problem and [2] I don't currently have the expertise to fix it.
